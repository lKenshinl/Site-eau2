           ...
           if(strlen($nom) <= 30)
           {
            if(strlen($prenom) <= 30)
            {
              if($age !=0)
              {
                          ...
                          $insertprt = $bdd->prepare("INSERT INTO particulier(numc, nom, prenom, age,civilite) VALUES(?, ?, ?, ?, ?)");
                          $insertprt->execute(array($getnumc, $nom, $prenom, $age, $civilite));
                          header("Location: validationcompte.php");
              }
              else
              {
                 echo "choisissez un âge plus élevé";
              }
            }
            else
            {
                    echo "Votre prenom ne doit pas dépasser 30 caractères ";
            }
           }
           else
           {
...
?>
							<form name="inscri1" method="post" action="crea2.php">
								...
								<div class="top-margin">
									...
									<input type="number" step="1" value="0" min="0" max="64" id="age" name="age">
								</div>
								<div class="top-margin">
								<p class="intro">civilité</p>
