			...
			<aside class="sidebar sidebar-left">
                ...
                ?>
                        ...
                        <div class="row">
							...
							<form method='post' action='sidebar-left.php'>
							<?php 
							$reqcommande=$bdd->prepare("SELECT * FROM commande WHERE numc = ? and statutcommande= 'nonvalide'");
							$reqcommande->execute(array($_SESSION['numc']));
							foreach($reqcommande as $infocommande)
                        {
							$numerocommande=$infocommande['numerocommande'];
						}
							if(isset($_POST['produit'.$i]))
										{
											$reqpossede = $bdd->prepare("SELECT * FROM possede WHERE numpro = ? AND numerocommande = ?");
											$reqpossede->execute(array($infoproduit['numpro'], $numerocommande));
											$commandeexist = $reqpossede->rowCount();
											if($commandeexist>0)
											{
												foreach($reqpossede as $infopossede)
                        						{
													$qtitep=$infopossede['quantite'];
												}
													$qtitep=$qtitep+1;
												$updateposs=$bdd->prepare("UPDATE possede SET quantite = ? WHERE numpro = ? AND numerocommande= ? AND statutp = 'nonvalide'" );
												$updateposs->execute(array($qtitep, $infoproduit['numpro'], $numerocommande));
											}
											$statutp='nonvalide';
											$qte=1;
											$insertprd = $bdd->prepare("INSERT INTO possede(numerocommande, numpro, quantite, statutp) VALUES(?, ?, ?, ?)");
											$insertprd->execute(array($numerocommande, $infoproduit['numpro'], $qte, $statutp));
										}
								?>
						...
						<?php
						    ...
						    {	
								...
								<div class="col-sm-6">
									...
									<img src=<?= $infoproduit['image'] ?> alt="Erreur de chargement de l'image">
									<p class="centre"> <u>Prix:</u> <?= $infoproduit['prix'] ?> €</p>
									<p>	<input class="btn btn-action" type="submit" value="Ajouter au panier" name="cart"></p>
								</div>
								<div class="col-sm-6">
									<header class="page-header">
										<h1 class="page-title-best">Découvrez l’eau <?= $infoproduit['nompro'] ?></h1>
